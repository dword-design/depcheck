...
function checkConfig(config, rootDir, includedDeps = []) {
  const parser = (0, _utils.wrapToArray)(config.parser);
  const plugins = (0, _utils.wrapToArray)(config.plugins).map(plugin => normalizePackageName(plugin, 'eslint-plugin'));
  const extendsArray = (0, _utils.wrapToArray)(config.extends);
  ...
  return _lodash.default.union(parser, plugins, presetPackages, presetDeps).filter(dep => !includedDeps.includes(dep));
}
